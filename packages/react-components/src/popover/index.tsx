...
const Popover = ({ children, overlay }: PopoverType) => {
  ...
  useEffect(() => {
    function handleClick(e: any) {
      if (!popoverRef?.current?.contains(e.target) && !popoverConetnRef?.current?.contains(e.target) && visible) {
        setVisible(false);
      }
    }
  ...
  return (
    <>
      ...
      {visible && (
        <div ref={popoverConetnRef} className="popover-placement" style={{ top: position.top, left: position.left }}>
